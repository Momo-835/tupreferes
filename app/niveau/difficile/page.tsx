...
export default function TuPreferesDifficile() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const { data, error } = await supabase
        ...
        .eq('difficultes', 3); // Niveau "difficile"
  ...
  const handleChoice = (choice: string) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      router.push(`/resultats?niveau=difficile&reponses=${encodeURIComponent(JSON.stringify(newAnswers))}`);
    }
  };
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white">
        Aucune question disponible pour le niveau difficile.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-red-500 to-purple-600 flex flex-col items-center justify-center p-4 text-white">
      <Card className="w-full max-w-lg">
        <CardHeader>
          <CardTitle className="text-center text-lg">Tu préfères... (Niveau Difficile)</CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              ...
              <Button
                ...
                className="w-full bg-purple-500 hover:bg-purple-600 py-3 text-lg"
              >
                {question.question2}
              </Button>
            </div>
