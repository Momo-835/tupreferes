...
export default function TuPreferesFacile() {
  ...
  const handleChoice = (choice: string) => {
    ...
    if (currentQuestion < questions.length - 1) {
      ...
      router.push(`/resultats?niveau=facile&reponses=${encodeURIComponent(JSON.stringify(newAnswers))}`);
    }
  };
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white">
        Aucune question disponible pour le niveau facile.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-400 to-blue-500 flex flex-col items-center justify-center p-4 text-white">
      <Card className="w-full max-w-lg">
        <CardHeader>
          <CardTitle className="text-center text-lg">Tu préfères... (Niveau Facile)</CardTitle>
        </CardHeader>
        <CardContent>
